(* $Id: debug.ml,v 1.1 2021-10-03 11:39:15-07 - - $ *)
...
let debug_progline (linenr, label'option, stmt'option) =
    fprintf stderr "DEBUG: %d %s: %s\n%!" linenr
        (string_of_option quote label'option)
        (string_of_option string_of_stmt stmt'option)

let debug_program (program : Absyn.program) =
    List.iter debug_progline program
